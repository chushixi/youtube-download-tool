...
| `--steam-rate` | Steam 原始價換算表格幣別的匯率乘數；留空=自動偵測（登入中國區帳號常回人民幣，會自動 ×`twd-rate`）。逐款 lite/full 模式無法自動偵測，中國區帳號請手動加 `--steam-rate 4.77` |

> **⚠ 登入 Steam 的回價幣別**：帶 `--steam-cookie` 時，Steam 會用**帳號所在地區的幣別**回價、忽略 `--currency`。中國區帳號會回**人民幣**。`search` 模式會自動偵測並換算成台幣；`lite`/`full` 模式請自行加 `--steam-rate 4.77`。
